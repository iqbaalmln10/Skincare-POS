...
>>>>>>> Stashed changes
  ...
  electron.ipcMain.handle("printer:list-ports", async () => {
    const { listPrinterPorts } = await Promise.resolve().then(() => require("./chunks/printer-CQrJ1stJ.js"));
    return listPrinterPorts();
  });
  electron.ipcMain.handle("printer:get-settings", async () => {
    const { getPrinterSettings } = await Promise.resolve().then(() => require("./chunks/printer-CQrJ1stJ.js"));
    return getPrinterSettings();
  });
  electron.ipcMain.handle("printer:save-settings", async (_event, comPort) => {
    const { savePrinterSettings } = await Promise.resolve().then(() => require("./chunks/printer-CQrJ1stJ.js"));
